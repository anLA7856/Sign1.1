...
public class SignDaoImpl implements SignDao{
	...
	public List<SignInfo> getNotSignInfoByStuId(String student_id) {
		...
		try {
			...
			while(rs.next()){
				...
				c.setSign_date(rs.getDate("sign_time").toString());
	...
	public List<SignNameInfo> getRealTimeSignNameInfo(String allow_sign_id) {
		...
		//先暂时用来测试
		String sql="SELECT student.`student_id`,signname.`sign_state`,student.`student_name`,signname.`sign_time` FROM signname,student WHERE allow_sign_id = "+allow_sign_id+" AND student.`student_id`=signname.`student_id`;";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
			rs = pstam.executeQuery();
			while(rs.next()){
				...
				Timestamp t = rs.getTimestamp("sign_time");
				Date d = new Date(t.getTime());
				c.setSign_time(d.toString());
	...
	public int getRealCountInOneSign(String allow_sign_id) {
		...
		int result = 0;
		//先暂时用来测试
		String sql="SELECT COUNT(course_id) AS mycount FROM signname WHERE allow_sign_id = "+allow_sign_id+";";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
			rs = pstam.executeQuery();
	...
	public List<StudentSignDetail> getSignListByCourseIdAndTeacherId(
			String student_id, String course_id) {
		PreparedStatement pstam = null;
		ResultSet rs = null;
		Connection conn = null;
		List<StudentSignDetail> list = new ArrayList<StudentSignDetail>();
		//先暂时用来测试
		String sql="SELECT signname.`sign_time`,signname.`sign_state` FROM signname WHERE signname.`course_id`="+course_id+" AND signname.`student_id`="+student_id+";";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
			rs = pstam.executeQuery();
