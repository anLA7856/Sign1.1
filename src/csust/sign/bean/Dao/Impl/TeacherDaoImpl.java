...
public class TeacherDaoImpl implements TeacherDao {
	...
	@Override
	public List<SignInfo> getTeacherSignInfoByTeacherID(String teacherID,
			String startCount, String count) {
		PreparedStatement pstam = null;
		ResultSet rs = null;
		Connection conn = null;
		...
		// 先暂时用来测试
		String sql = "SELECT allow_sign.`alow_sign_id`,course.`course_name`,allow_sign.`sign_time`,course.`course_id`,teacher.`teacher_name`,allow_sign.`target` FROM course,teacher,allow_sign WHERE course.`course_id`=allow_sign.`course_id` AND teacher.`teacher_id`="
				+ teacherID
				+ " AND course.`teacher_id`=teacher.`teacher_id` AND allow_sign.`target`=1 LIMIT "
				+ startCount + "," + count + ";";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
	...
	public int addTeacherUserInfo(String name, String username,
		...
		int result = 0;
		// 先暂时用来测试
		String sql = "INSERT INTO teacher(teacher_name,teacher_username,teacher_password,teacher_wifimac) VALUES('"
				+ name
				+ "','"
				+ username
				+ "','"
	...
	public int validateNewUserInfo(String username) {
		...
		int result = 0;
		// 先暂时用来测试
		String sql = "SELECT COUNT(*) AS COUNT FROM teacher WHERE teacher_username='"
				+ username + "';";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
	...
	public int updateWifiMacAddress(String teacher_username, String wifiMac) {
		...
		int result = 0;
		// 先暂时用来测试
		String sql = "UPDATE teacher SET teacher.`teacher_wifimac`='" + wifiMac
				+ "' WHERE teacher.`teacher_username`='" + teacher_username
				+ "';";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
	...
	@Override
	public List<TeacherListInfo> getTeaListByStudentId(String student_id,
		...
		// 先暂时用来测试
		String sql = "SELECT DISTINCT teacher.`teacher_id`,teacher.`teacher_username`,teacher.`teacher_name`FROM teacher,course,student_course WHERE teacher.`teacher_id`=course.`teacher_id` AND course.`course_id`=student_course.`course_id` AND student_course.`student_id`="
				+ student_id + " LIMIT " + start + "," + count + ";";
		try {
			conn = ConnectFactory.getConnection();
			pstam = conn.prepareStatement(sql);
			rs = pstam.executeQuery();
	...
	public int modifyTeacherPassword(String teacher_id, String mynew) {
		...
		int result = 0;
		// 先暂时用来测试
		String sql = "UPDATE teacher SET teacher_password='" + mynew
